...
async function getNetworkBooks(bookName) {
  const url = `https://www.googleapis.com/books/v1/volumes?printType=books&q=${bookName}`
  const booksFromApi = (await axios.get(url)).data.items
...
function addGoogleBook(bookToAdd) {
  const book = {
    id: bookToAdd.id || Utils.getRandomId(),
...
function get(bookId) {
  ...
  return DbService.get(KEY, bookId);
}

function remove(bookId) {
  return DbService.delete(KEY, bookId);
}
...
function addReview(reviewToAdd, book) {
  console.log('hhhhh');
  ...
  return DbService.put(KEY, copyBook)
}
function deleteReview(reviewId, book) {
  console.log('reviewid service', reviewId);
  const copyBook = { ...book }
...
async function getNextBook(bookId) {
  ...
  console.log('books get next',books);
  const nextBook = books.findIndex(book => book._id === bookId)
  console.log('next',nextBook);
  if(nextBook+1===books.length)return books[0]._id;
  else return books[nextBook + 1]._id
  
}
async function getPrevBook(bookId) {
  const books= await query()
  console.log('books get prev',books);
  const prevBook = books.findIndex(book => book._id === bookId)
  if(prevBook===0)return books[books.length-1]._id
  return books[prevBook - 1]._id
